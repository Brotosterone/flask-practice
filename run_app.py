...
def build_username(first_name,last_name):
    ...
    """
    name = '{}{}'.format(first_name[0].lower(),last_name.lower())
    return name
...
def search_user():
    ...
    r_list  = [person for person in users if search.lower() in person.lower()]
    return 'Found {} users that match with search: "{}"'.format(len(r_list), search)
